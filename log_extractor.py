...
def next_lines(filename, n, next_num_lines):
    with open(filename) as f:
        return collections.deque(f, n, next_num_lines)
    ...
    with open(args.input_file.name) as input_file:
        ...
        for line in input_file:
            ...
            if message_type in line:
                ...
                log_lines.clear()
            log_lines.append(next_lines(args.input_file.name, n=i+1, next_num_lines=data[message_type]["after"]))
            for _ in range(0, data[message_type]["after"]):
                output_file.write(log_lines.pop())
            else:
                log_lines.append(line)
